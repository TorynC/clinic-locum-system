...
export default function JobDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const postJobApplication = async() => {
    ...
    try {
      const result = await axiosInstance.post("/post-job-application", payLoad, {
        headers: {
          ...(token && { Authorization: `Bearer ${token}` }),
        }
      })
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Job Details</h1>
              ...
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <h1 className="text-l">{clinicName}</h1>
            ...
            <CardContent>
              <div className="flex flex-wrap gap-6 py-6 border-y border-slate-200">
                ...
                <div className="flex items-center">
                  ...
                  {!job?.two_rates && <div>
                    ...
                    <p className="font-semibold text-slate-900">{`${job?.start_time.split(":")[0]}:${job?.start_time.split(":")[1]} - ${job?.end_time.split(":")[0]}:${job?.end_time.split(":")[1]}`}</p>
                  </div>}

                  {job?.two_rates && <div>
                    <p className="text-xs font-medium text-slate-500">DAY SHIFT</p>
                    <p className="font-semibold text-slate-900">{`${job?.start_day_time.split(":")[0]}:${job?.start_day_time.split(":")[1]} - ${job?.end_day_time.split(":")[0]}:${job?.end_day_time.split(":")[1]}`}</p>
                  </div>}
                  
                </div>

                {job?.two_rates && <div className="flex items-center">
                  <Clock className="h-5 w-5 text-red-600 mr-2" />
                    <div>
                    <p className="text-xs font-medium text-slate-500">NIGHT SHIFT</p>
                    <p className="font-semibold text-slate-900">{`${job?.start_night_time.split(":")[0]}:${job?.start_night_time.split(":")[1]} - ${job?.end_night_time.split(":")[0]}:${job?.end_night_time.split(":")[1]}`}</p>
                  </div>
                </div>}
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-slate-900">{job?.duration} hours</p>
                  </div>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  {!job?.two_rates && <div>
                    ...
                    <p className="font-semibold text-slate-900">RM {job?.rate}/hr</p>
                  </div>}
                  {job?.two_rates && <div>
                    <p className="text-xs font-medium text-slate-500">HOURLY RATE</p>
                    <p className="font-semibold text-slate-900">RM {job?.day_rate}/hr</p>
                  </div>}
                </div>
                ...
                {job?.two_rates && <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-xs font-medium text-slate-500">NIGHT HOURLY RATE</p>
                    <p className="font-semibold text-slate-900">RM {job?.night_rate}/hr</p>
                  </div>
                </div>}
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-xs font-medium text-slate-500">TOTAL PAY</p>
                    <p className="font-semibold text-green-600">RM {job?.total_pay}</p>
                  </div>
                </div>

                <div className="flex items-center">
                  <Building className={cn("h-5 w-5 mr-2", 
                    job?.gender === "female" ? "text-pink-600" :
                    job?.gender === "male" ? "text-blue-600" :
                    "text-slate-600" )} />
                  <div>
                    <p className="text-xs font-medium text-slate-500">GENDER PREF</p>
                    <p
                      className={cn(
                        "font-semibold",
                        job?.gender === "female"
              ...
              <div className="space-y-6 mt-8">
                ...
                <div className="bg-red-50 rounded-2xl p-6">
                  ...
                  <p className="text-slate-700 leading-relaxed">{job?.incentives}</p>
                </div>
                ...
                <div className="bg-blue-50 rounded-2xl p-6">
                  ...
                  <p className="text-slate-700 leading-relaxed">{job?.special_instructions}</p>
                </div>
                  ...
                  <div className="bg-emerald-50 rounded-2xl p-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {job?.procedure.map((skill: string) => (
                        <Badge key={skill} className="bg-emerald-100 text-emerald-700 border-emerald-200 font-medium">
                          {skill}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <div className="bg-orange-50 rounded-2xl p-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {job?.languages.map((language: string) => (
                        <Badge key={language} className="bg-orange-100 text-orange-700 border-orange-200 font-medium">
                          {language}
                        </Badge>
                      ))}
                    </div>
          ...
          <Card className="border-0 shadow-medium bg-gradient-to-br from-white to-slate-50/50">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                <Button className="w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-semibold py-3 rounded-xl shadow-lg"
